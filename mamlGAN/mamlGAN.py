...
def fast_adapt(batch, learner, generator, optimizer2 , train_ratio ,loss, loss1 ,adaptation_steps, shots, ways, device, test):
    ...
    for step in range(adaptation_steps):
        ...
        noise1 = torch.randn((adaptation_data.shape[0], 32)).to(device)
    
        inp1 = torch.cat((en1,noise1)  , dim=1 ).type(torch.cuda.FloatTensor)

        noise2 = torch.randn((adaptation_data.shape[0], 32)).to(device)
        ...
        if alpha :
          ...
          for i in range(train_ratio):
            ...
            _ , last_logit1 = learner_g(fake_data1.reshape(-1,16))
        ...
        _ , last_logit3 = learner(fake_data2.reshape(-1,16))
...
def main(
    ...
    np.savetxt( '/home/sever2users/Desktop/MetaGAN/mamlGAN/test_results/{}way_{}shot_{}train_adp_steps.txt'.format(ways,shots,adaptation_steps), np.array(x))

    torch.save(maml_test.module.state_dict(), '/home/sever2users/Desktop/MetaGAN/mamlGAN/saved_models/state_dict_{}way_{}shot_{}train_adp_steps.pth'.format(ways,shots,adaptation_steps) )  
