...
class dataset(torch.utils.data.Dataset):    
    def __init__(self , train=True):
        self.data= pd.read_csv("/home/sever2users/Desktop/MetaGAN/datasets/abalone.csv").to_numpy()

        self.data = np.delete(self.data , np.where(self.data[:,-1] <3 ) , axis=0)
        self.data = np.delete(self.data , np.where(self.data[:,-1] >21 ), axis=0)
        
        if train :
          self.data = self.data[self.data[:,-1]<16]
        else :
          self.data = self.data[self.data[:,-1]>=16]
    ...
    for step in range(adaptation_steps):
        ...
        if alpha :
          ...
          for i in range(train_ratio):
            ...
            _ , last_logit1 = learner_g(fake_data1.reshape(-1,10))
        ...
        _ , last_logit3 = learner(fake_data2.reshape(-1,10))
...
def main(
    ...
    np.savetxt( '/home/sever2users/Desktop/MetaGAN/mamlGAN/test_results/abalone/{}way_{}shot_{}train_adp_steps.txt'.format(ways,shots,adaptation_steps), np.array(x))

    torch.save(maml_test.module.state_dict(), '/home/sever2users/Desktop/MetaGAN/mamlGAN/saved_models/abalone/state_dict_{}way_{}shot_{}train_adp_steps.pth'.format(ways,shots,adaptation_steps) )  
